...
q('#adminLoginForm')?.addEventListener('submit',async e=>{
 ...
 try{
  ...
  if(me.profile?.role!=='admin'){
   ...
   sessionStorage.removeItem('wt_admin_gate');
   flash('Esta conta não possui permissão administrativa.','bad');
   return
  }
  sessionStorage.setItem('wt_admin_gate','1');
...
sessionStorage.removeItem('wt_admin_gate');
const qs=new URLSearchParams(location.search);if(qs.get('denied')==='1')flash('Acesso restrito ao administrador.','bad');
