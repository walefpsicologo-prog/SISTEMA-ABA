...
export async function currentUser(){
 const {data,error}=await timed(sb.auth.getSession(),8000,'auth_timeout');
 if(error)throw error;
 const user=data?.session?.user||null;
 if(!user)return{user:null,profile:null};
 let profile=null;
 try{const r=await timed(sb.from('profiles').select('*').eq('id',user.id).maybeSingle(),8000,'profile_timeout');if(!r.error)profile=r.data||null}catch(err){console.warn('profile_load_failed',err)}
 return{user,profile};
}
export async function activeEnrollments(){const{data,error}=await timed(sb.from('enrollments').select('*,courses(*)').in('status',['active','completed']).order('created_at',{ascending:false}));if(error)throw error;return data||[]}
export async function enrollmentForCourse(courseId){const{data,error}=await timed(sb.from('enrollments').select('*,courses(*)').eq('course_id',courseId).in('status',['active','completed']).maybeSingle(),10000,'enrollment_timeout');if(error)throw error;return data||null}
