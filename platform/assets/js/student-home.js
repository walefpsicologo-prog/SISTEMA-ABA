...
import {esc,dt,flash,userError,currentUser,activeEnrollments,publicCourses,salePrice,money,q} from './api.js';
...
const ownedCard=e=>`<article class="student-course-card"><div><span class="badge-access">${esc(sourceLabel(e.source))}</span><h2>${esc(e.courses?.title||'Curso')}</h2><p>${esc(e.courses?.subtitle||e.courses?.description||'')}</p>${e.access_ends_at?`<p><strong>Acesso até:</strong> ${new Intl.DateTimeFormat('pt-BR',{dateStyle:'medium'}).format(new Date(e.access_ends_at))}</p>`:''}</div><div class="actions"><a class="btn" href="/curso-aluno.html?curso=${encodeURIComponent(e.course_id)}">Entrar no curso</a></div></article>`;
...
async function boot(){
  try{
    ...
    let catalog=[];
    try{catalog=await publicCourses()}catch(err){console.error('student_catalog',err)}
    const ownedIds=new Set(enrollments.map(e=>e.course_id));
    const available=(catalog||[]).filter(c=>!ownedIds.has(c.id));

    const blocks=[];
    if(enrollments.length){
      blocks.push(enrollments.map(ownedCard).join(''));
    }
    if(available.length){
      if(enrollments.length)blocks.push('<div style="grid-column:1/-1;margin-top:8px"><span class="kicker">Outros cursos disponíveis</span></div>');
      blocks.push(available.map(availableCard).join(''));
    }
