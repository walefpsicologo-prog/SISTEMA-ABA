...
function verificationState(p){
  const expected=p.expected_item_count==null?null:Number(p.expected_item_count);
  const verified=Number(p.source_verified_criteria||0);
  const hasSource=p.source_status==='user_provided_source'||p.protocol_kind==='source_protocol_map'||sources.some(s=>s.protocol_id===p.id);
  if(hasSource&&expected!=null&&expected>0&&verified>=expected)return {label:'Fonte completa conferida',cls:'verified',complete:true};
  if(hasSource&&verified>0)return {label:'Fonte parcialmente conferida',cls:'partial',complete:false};
  if(hasSource)return {label:'Fonte vinculada · itens não conferidos',cls:'partial',complete:false};
  if(p.source_status==='operational_complete')return {label:'Matriz clínica operacional ampliada',cls:'operational',complete:false};
  if(p.source_status==='operational_focused')return {label:'Módulo clínico operacional',cls:'operational',complete:false};
  return {label:'Biblioteca clínica operacional',cls:'operational',complete:false};
}
function statusLabel(p){
  if(p.protocol_kind==='source_protocol_map')return ['Mapa documental','map'];
  if(p.source_status==='user_provided_source')return ['Documento-fonte vinculado','map'];
  if(p.source_status==='operational_complete')return ['Matriz operacional','operational'];
...
function patchCards(){
  ...
  view.querySelectorAll('.aba-card').forEach(card=>{
    ...
    const [lab,cls]=statusLabel(p),v=verificationState(p);const box=document.createElement('div');box.className='aba-source-badges';
    box.innerHTML=badge(lab,cls)+badge(v.label,v.cls)+(p.group_name?badge(p.group_name):'');
...
async function patchDetail(){
  ...
  const src=sources.filter(x=>x.protocol_id===p.id),sec=sections.filter(x=>x.protocol_id===p.id);const [lab,cls]=statusLabel(p),v=verificationState(p);
  ...
  let sourceText='Este protocolo pertence à biblioteca operacional clínica e não é apresentado como reprodução integral de um instrumento comercial.';
  if(src.length||p.source_status==='user_provided_source'||p.protocol_kind==='source_protocol_map'){
    sourceText=v.complete
      ? 'A quantidade esperada e os itens marcados como verificados coincidem. Esta é a única situação em que o sistema classifica a fonte como completa e conferida.'
      : 'Existe documento-fonte vinculado, porém o sistema não declara completude oficial enquanto todos os itens esperados não estiverem individualmente conferidos. Critérios clínicos operacionais permanecem separados da fonte.';
  }
  const box=document.createElement('div');box.className='aba-source-panel';
  box.innerHTML=`<div class="aba-source-badges">${badge(lab,cls)}${badge(v.label,v.cls)}${p.group_name?badge(p.group_name):''}</div><h4>Origem, completude e fidelidade</h4><p>${esc(sourceText)}</p>${sourceFile?`<p><b>Arquivo-fonte:</b> <span class="aba-source-file">${esc(sourceFile)}</span></p>`:''}<div class="aba-source-grid"><div class="aba-source-stat"><span>Critérios ativos</span><b>${actual}</b></div><div class="aba-source-stat"><span>Itens esperados na fonte</span><b>${expected}</b></div><div class="aba-source-stat"><span>Itens transcritos/verificados</span><b>${verified}</b></div><div class="aba-source-stat"><span>Camada operacional</span><b>${oper}</b></div></div>${sec.length?`<details class="aba-source-structure"><summary>Ver estrutura documental (${sec.length} áreas/seções)</summary><div class="aba-source-sections">${sec.map(s=>`<div class="aba-source-section"><b>${esc(s.section_code)} · ${esc(s.section_name)}</b>${s.item_range?`<br>${esc(s.item_range)}`:''}${s.expected_item_count!=null?` · ${fmt(s.expected_item_count)} itens`:''}${s.source_verified?' · estrutura conferida':' · referência estrutural'}</div>`).join('')}</div></details>`:''}${p.source_notes?`<p>${esc(p.source_notes)}</p>`:''}${p.official_notice?`<p><b>Aviso técnico:</b> ${esc(p.official_notice)}</p>`:''}`;
  ...
  panel.querySelectorAll('#abaCriteriaRows tr').forEach(tr=>{const td=tr.querySelector('td');if(!td)return;const code=td.textContent.trim();const c=byCode.get(code);if(!c)return;const tag=document.createElement('span');tag.className='aba-source-row-tag '+(c.source_verified?'verified':'operational');tag.textContent=c.source_verified?'Fonte verificada':'Operacional';td.appendChild(tag)});
}
