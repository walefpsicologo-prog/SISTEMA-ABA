...
async function renderForum(){const{data,error}=await sb.from('forum_threads').select('*').eq('course_id',C.course.id).eq('archived',false).order('pinned',{ascending:false}).order('created_at',{ascending:false});if(error)return flash(userError(error),'bad');q('#forumThreads').innerHTML=data?.length?data.map(t=>`<button class="forum-thread" data-thread="${t.id}"><span>${t.pinned?'FIXADO · ':''}${t.locked?'BLOQUEADO · ':''}${dateOnly(t.created_at)}</span><strong>${esc(t.title)}</strong><p>${esc(t.body)}</p></button>`).join(''):'<div class="empty-state">Nenhum tópico publicado.</div>';qa('[data-thread]').forEach(b=>b.addEventListener('click',()=>openThread(b.dataset.thread)))}
